...
class Curl extends NapiCurl {
	 ...
	 */
	async perform ({ post, put, signal } = {}) {
		if (post !== undefined && put !== undefined)
		...
		this.queue = new Promise(resolve => release = resolve).then(() => undefined);
		await prev;
		...
		return new Promise( (resolve, reject) => {
			...
			if (signal) signal.onabort = () => super.cancel();
			super.perform();
		});
	}
	 ...
	 */
	get (url, { signal } = {}) {
		return this.setOpt({ HTTPGET: 1, URL: url.toString() }).perform({ signal });
	}
	 ...
	 */
	head (url, { signal } = {}) {
		return this.setOpt({ NOBODY: 1, URL: url.toString() }).perform({ signal });
	}
	 ...
	 */
	delete (url, { signal } = {}) {
		return this.setOpt({ CUSTOMREQUEST: 'DELETE', URL: url.toString() }).perform({ signal });
	}
	 ...
	 */
	put (url, { body, signal } = {}) {
		return this.setOpt({ URL: url.toString() }).perform({ put: body, signal });
	}
}
