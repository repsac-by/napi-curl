...
class Curl extends NapiCurl {
	...
	perform (opts = {}) {
		return new Promise( (resolve, reject) => {
			...
			const onStreamError = () => super.cancel();
			...
			super.onHeader = header => headers.push(header);

			super.onData = buf => {
				resolve(new Response(this.getInfo.bind(this), headers, stream));
				const push = super.onData = buf => { stream.push(new Uint8Array(buf.slice(0))) || this.readStop() };
				push(buf);
			};
