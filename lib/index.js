...
class Curl extends NapiCurl {
	...
	perform (opts = {}) {
		return new Promise( (resolve, reject) => {
			...
			stream.once('error', () => super.cancel());

			super.onHeader = headers => {
				resolve(new Response(this.getInfo.bind(this), headers, stream));
				super.onHeader = trailers => console.log(trailers);
			};

			super.onError = err => { reject(err); stream.destroy(err) };
			super.onData = buf => { stream.push(new Uint8Array(buf.slice(0))) || this.readStop() };
			super.onEnd = () => { stream.push(null) };
